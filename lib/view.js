...
_.extend(View.prototype, Events, {
  ...
  $: function(selector) {
    return Backbone.$ ? this.$el.find(selector) : slice.call(this.el.querySelectorAll(selector));
  },
  ...
  delegateEvents: function(events) {
    ...
    for (var key in events) {
      ...
      if (Backbone.$) {
        ...
        var types = eventName.trim().split(" ");
        types.forEach(function(type) {
          ...
          var handler = {
            ...
            method: function(event) {
              ...
              if (this.el === target && target.handlers.some(function(h) { return h.cid === this.cid }, this)) {
                method(event);
              }
            }
          }
          handler.method = handler.method.bind(handler);

          el.handlers.push(handler);
          el.addEventListener( name, handler.method, false );
        }, this);
      }
    }
    return this;
  ...
  undelegateEvents: function() {
    if (Backbone.$) {
      ...
      var handlers = this.el.handlers || (this.el.handlers = []);

      handlers.forEach(function(handler, i) {
        if (handler.cid !== this.cid) return;
        this.el.removeEventListener(handler.name, handler.method, false);
        el.handlers.splice(i, 1);
      }, this);
    }
